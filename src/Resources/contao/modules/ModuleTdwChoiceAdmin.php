...
class ModuleTdwChoiceAdmin extends \BackendModule
{
			...
			if(file_exists($strCsvFile)) {
				...
				while(($data = fgetcsv($handle,1000,$delimiter)) !== false) {
					if ($row > 1 && 
						...
						$data[3] == date('d.m.Y',$student['dateOfBirth'])
					) {
	...
	protected function importLecturesFromCsv() {
		...
		while(($data = fgetcsv($handle,1000,$delimiter)) !== false) {
			...
			if($row > 1) {
				...
				for($c = 0; $c < $num; $c++) array_push($arrColumns,$data[$c]);
				
				/*Neue Variante mit Speaker*/ 
				for ($i = 0; $i < 8; $i++) $bn = $bn + pow(2,$i) * !!$arrColumns[$i + 4];
	...
	protected function copyLecturesTo($articleId) {
		...
		while ($arrLecture = $lectures->fetchAssoc()) {
			...
			if($arrLecture['speaker']) $strHeadline = $arrLecture['speaker'] . ': ' . $arrLecture['title'];
			else $strHeadline = $arrLecture['title'];
